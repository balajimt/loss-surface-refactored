...
def eval_avalanche(loader, model, criterion):
    loss_sum = 0.0
    correct = 0.0
    ...
    for i, (input, target, _) in enumerate(loader):
        ...
        pred = output.data.max(1, keepdim=True)[1]
        correct += pred.eq(target_var.data.view_as(pred)).sum().item()
        all_target_variables.append(target_var)
...
def train_epoch_avalanche(loader, model, criterion, optimizer):
    loss_sum = 0.0
    correct = 0.0
...
def train_epoch_volume_avalanche(loader, model, criterion, optimizer, vol_reg,
                       nsample):
    loss_sum = 0.0
    correct = 0.0
