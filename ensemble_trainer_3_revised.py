...
def main(args):
    for i in range(10):
        print("Currently executing ", i, "th iteration")
        ...
        with open('spiral_dataset//train_points.pkl','rb') as f:
            train_points = pickle.load(f)
        with open('spiral_dataset//train_labels.pkl','rb') as f:
            train_labels = pickle.load(f)
        with open('spiral_dataset//test_points.pkl','rb') as f:
            test_points = pickle.load(f)
        with open('spiral_dataset//test_labels.pkl','rb') as f:
        ...
        # TODO: Start from the same fixed points (start from the same starting point)
        for component in range(args.n_component):
            fix_pts = [False]
            simplex_model = SimplexNet(2, SpiralModel, n_vert=1, fix_points=fix_pts)
            simplex_model.load_state_dict(torch.load('input_weights/base_0_simplex_0.pt'))
            ...
            for vv in range(args.n_verts):
                ...
                logger.info("Saving to " + str(os.path.join(OUTPUT_WEIGHTS, fname)))
                torch.save(checkpoint, os.path.join(OUTPUT_WEIGHTS, fname))
        ...
        for name, param in simplex_model.named_parameters():
            logger.info(name)
        logger.info("Saving simplex model state dict to " + str(os.path.join(OUTPUT_WEIGHTS, 'state_dict_test_' + CURRENT_TIME + '.pth')))
